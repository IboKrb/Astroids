...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y, player_speed_x, player_speed_y, angle) -> None:
        super().__init__()
        self.image = pygame.image.load(Settings.get_image("bullet.png")).convert_alpha()
        self.image = pygame.transform.scale(self.image,(Settings.bullet_size, Settings.bullet_size))
    ...
    def change(self):
        self.angle_ = 0
        self.angle_ = (math.pi/180) * self.angle
    ...
    def speed(self):
        self.change()
        self.speed_x = self.speed_x - math.sin(self.angle_)
        self.speed_y = self.speed_y - math.cos(self.angle_)                
...
class Game:
    ...
    def draw(self) -> None:
        ...
        self.bullets.draw(self._screen)
        pygame.display.flip()

    def shoot(self):
        if len(self.bullets.sprites()) < 10:
            self.bullets.add(Bullet(self._ship.rect.centerx, self._ship.rect.centery,
                             self._ship.speed_x, self._ship.speed_y, self._ship._angle))
